 ...
 *
 * v4 — Template ESP32-S3 per a Arduino IDE
 * Funcionalitats: WiFi multi-xarxa · MQTT · Neopixel · Temperatura interna + DS18B20
 *                 Telemetria unificada (ssid + temperatures dins d'estat)
 * Arquitectura:   4 tasques FreeRTOS (loop() queda buit)
 *
 * Canvis respecte v3:
 *   - Les temperatures (interna i DS18B20) ja no es publiquen en tòpics separats.
 *     Es llegeixen igualment cada INTERVAL_TEMP ms i s'inclouen al JSON d'estat.
 *   - El JSON d'estat incorpora ara: ssid, tempInt, tempDS.
 *   - Resultat: menys tòpics MQTT, missatge d'estat autosuficient.
 *
...
// Intervals (mil·lisegons)
#define INTERVAL_ESTAT  5000    // freqüència de publicació del JSON d'estat
#define INTERVAL_TEMP   10000   // freqüència de lectura dels sensors de temperatura
...
//   /esp32s3/alumneXX/estat  → cada 5s
//     {"ssid":"...","ip":"...","rssi":-xx,"uptime":xxxxx,"tempInt":xx.xx,"tempDS":xx.xx,"leds":[...]}
// ============================================================
...
// Últimes lectures de temperatura (actualitzades per les tasques de sensor)
volatile float tempInt = 0.0f;   // temperatura interna ESP32-S3
volatile float tempDS  = 0.0f;   // temperatura DS18B20 extern
...
//  Llegeix el sensor tèrmic intern de l'ESP32-S3 cada
//  INTERVAL_TEMP ms i desa el valor a tempInt.
...
void tempInternaTask(void* param) {
  ...
  while (true) {
    ...
    float t = 0.0f;
    temperature_sensor_get_celsius(hTempSensor, &t);
    tempInt = t;
...
//  Llegeix el sensor extern a GPIO16 cada INTERVAL_TEMP ms
//  i desa el valor a tempDS.
//  Prioritat 4 · Core 1 · Stack 4096
// ============================================================
  ...
  while (true) {
    ...
    if (mqtt.isConnected() && (millis() - tEstat) >= INTERVAL_ESTAT) {
      ...
      snprintf(json, sizeof(json),
        "{\"ssid\":\"%s\",\"ip\":\"%s\",\"rssi\":%d,\"uptime\":%lu"
        ",\"tempInt\":%.2f,\"tempDS\":%.2f"
        ",\"leds\":[%d,%d,%d,%d]}",
        WiFi.SSID().c_str(),
