 ...
 *
 * v8 — Template ESP32-S3 per a Arduino IDE
 * Funcionalitats: WiFi multi-xarxa · MQTT · Neopixel · Temperatura interna + DS18B20 · NTP · LWT · Polsadors avançats + actuadors
...
uint32_t  polsadorPressStart[4] = {0, 0, 0, 0};
bool      polsadorLongFired[4]  = {false, false, false, false};
...
// ---- Helper: toggle LED des de polsadorsTask ----
void toggleLedLocal(int idx) {
  bool val;
  ...
  Serial.printf("[POLS] LED%d toggle → %d\n", idx + 1, val);
}

// ---- Accions per polsador ----
...
void onLongPress(int boto) {
  switch (boto) {
    case 2: toggleLedLocal(2); break;  // Polsador 2 long → LED 3
    default: break;
  }
}
...
// ============================================================
//  TASCA: Polsadors avançats
//  Llegeix els 4 polsadors (GPIO 39-42) amb pull-up intern i
//  debounce per software. Detecta pulsació curta i llarga.
//  Pols1: curt → toggle LED1 · Pols2: curt → LED2, llarg → LED3
//  Pols3/4: només events MQTT.
//  Publica events PREMUT i ALLIBERAT per MQTT.
//  Prioritat 3 · Core 1 · Stack 4096
// ============================================================
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (polsadorTemps[i] && (millis() - polsadorTemps[i]) >= DEBOUNCE_MS) {
        ...
        if (estable != polsadorAnt[i]) {
          ...
          if (estable == LOW) {
            // === Polsador premut (confirmat) ===
            polsadorPressStart[i] = millis();
            polsadorLongFired[i]  = false;
            ...
            // === Polsador alliberat (confirmat) ===
            if (!polsadorLongFired[i]) {
              onShortPress(i + 1);
            }
      ...
      // Detectar pulsació llarga mentre es manté premut
      if (polsadorAnt[i] == LOW && polsadorPressStart[i] &&
          !polsadorLongFired[i] &&
          (millis() - polsadorPressStart[i]) >= LONG_PRESS_MS) {
        polsadorLongFired[i] = true;
        onLongPress(i + 1);
        Serial.printf("[POLS] Polsador %d LONG PRESS\n", i + 1);
      }
    }
  }
}
...
void setup() {
  ...
  xTaskCreatePinnedToCore(polsadorsTask,          "pols",  4096, NULL,  3, NULL, 1);
